...
namespace render
{
	...
	Image::Image(const Global& global, VkFormat format, Extent extent, uint32_t layer_cnt) : LazyRenderObj(global), format_(format), extent_(extent), holds_external_handle_(false), usage_(0), layer_cnt_(layer_cnt), mipmap_levels_count_(1)
	{
	...
	Image::Image(const Global& global, VkFormat format, VkImage image_handle) : LazyRenderObj(global), format_(format), mipmap_levels_count_(1), holds_external_handle_(true), usage_(0), extent_(0, 0)
	{
		handle_ = image_handle;
	}

	Image::Image(const Global& global, BuiltinImageType type) : Image(global, global.color_format, { 1, 1 })
	{
		...
		switch (type)
		{
		case Image::BuiltinImageType::kBlack:
			data = black;
			break;
		case Image::BuiltinImageType::kWhite:
			data = white;
			break;
		case Image::BuiltinImageType::kNormal:
			data = normal;
			break;
		case Image::BuiltinImageType::kError:
			image_size = 4 * 7 * 7;
			extent_ = { 7, 7 };
			data = error;
			break;
	...
	Image::Image(const Global& global, VkFormat format, Extent extent, const unsigned char* pixels) : Image(global, format, extent)
	{
	...
	Image Image::FromFile(const Global& global, const std::string_view& path)
	{
		int width = 0;
		int height = 0;
		int channels;
	...
	void Image::TransitionImageLayout(const CommandPool& command_pool, TransitionType transfer_type) const
	{
	...
	void Image::CopyBuffer(const CommandPool& command_pool, const Buffer& buffer) const
	{
	...
	//const ImagePropertiesStorage& Image::GetImageProperties() const
	//{
	//	return image_properties_;
	//}


	uint32_t Image::GetMipMapLevelsCount() const
	{
		return mipmap_levels_count_;
	}

	uint32_t Image::AddUsageFlag(uint32_t flag) const
	{
		assert(handle_ == VK_NULL_HANDLE);
		usage_ = usage_ | flag;
		return usage_;
	...
	uint32_t Image::CheckUsageFlag(uint32_t flag) const
	{
		return usage_ & flag;
	}
